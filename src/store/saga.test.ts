...
import { load, loadItemMetadata, fetchMore, limit } from './saga';
...
describe('feed saga', () => {
  describe('loadItemMetadata', () => {
    it('should load metadata for item', async () => {
      ...
      const initialState = {
        feed: {
          value: {
            entities: { 'the-item-id': { id: 'the-item-id', metadataUrl } },
          },
        },
      };
    ...
    it('should update item in state with metadata', async () => {
          ...
          value: {
            ...
            entities: {
              'the-first-item-id': {
                ...
                metadataUrl: 'first-metadata-url',
                name: 'cats',
                description: 'what',
              },
              'the-second-item-id': {
      ...
      } = await expectSaga(loadItemMetadata, { payload: 'the-second-item-id' })
        .withReducer(rootReducer, initialState)
        .provide([[matchers.call(metadataService.load, metadataUrl), metadata]])
        .call(metadataService.load, metadataUrl)
        .run();
  ...
  describe('load', () => {
    ...
    it('should get feed and item for provided payload', async () => {
      ...
      await expectSaga(load, { payload: { route: 'theroute', provider: {} } })
        ...
        .call([znsClient, znsClient.getFeed], 'the-id', limit)
    ...
    it('should set feed children in store from zns client', async () => {
      ...
      await expectSaga(load, { payload: { route: '', provider: {} } })
        .withReducer(reducer)
        .provide([[matchers.call.fn(client.get), znsClient]])
        .hasFinalState({
          value: expectedValue,
  ...
  describe('fetchMore', () => {
    ...
    it('should pass current provider to zns client', async () => {
      ...
      await expectSaga(fetchMore, { payload: { route: '', provider } })
        .provide([[matchers.call.fn(client.get), getZnsClient()]])
        .call(client.get, provider)
        .run();
    });
    ...
    it('should get feed for provided payload', async () => {
      ...
      await expectSaga(fetchMore, {
        payload: { route: 'theroute', provider: {}, offset: 0 },
      })
        .provide([
          [matchers.call.fn(client.get), znsClient],
          [
        ...
        .call([znsClient, znsClient.getFeed], 'the-id', limit, 0)
        .run();
    });
    ...
    it('should set hasMore to false on case of there is no items', async () => {
      const znsClient = getZnsClient({
        getFeed: async () => [],
      });
